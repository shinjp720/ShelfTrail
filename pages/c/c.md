...
| 8 | == | 等価 | → |
| 8 | != | 非等価 | → |
| 9 | & | ビット積 共に1なら1、どちらかが0なら0 | → |
| 10 | ^ | ビット差 異なれば1、同じであれば0 | → |
| 11 | \| | ビット和 どちらかが1なら1 | → |
| 12 | && | 論理積 | → |
| 12 | \|\| | 論理和 | → |
| 13 | ?: | 三項演算子 | ← |
...
| 演算子 | 意味                                                     | 使い方 |
| ----- | ------------------------------------------------------- |-------|
| \|    | ビット和 (**OR**)<br>2つのビット列を比較して、どちらかが1なら1、共に0なら0 (フラグを立てる)                   | x \| y |
| ^     | ビット差 (**XOR**)<br>2つのビット列を比較して、異なれば1、同じであれば0 (ビットをトグルする)        | x ^ y |
| &     | ビット積 (**AND**)<br>2つのビット列を比較して、共に1なら1、どちらかが0なら0 (立っているビットの確認や、反転ビットを倒す処理) | x & y |
| ~     | 補数。ビット反転 (**NOT**)<br>xのビット列の、0と1を反転                     | ~x |
| <<    | ビット左シフト xをy回左にシフト                              | x << y |
| >>    | ビット右シフト xをy回右にシフト                              | x >> y |
...
// 初期化の場合は↓でもOK
unsigned int flag = (FLAG_A | FLAG_C); // = か |= の違い
```
